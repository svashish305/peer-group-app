	...
	useEffect(
		() => {
			async function fetchData() {
				const userToEdit = await API.getUserById(
					props.userId,
					token['pg-token']
		...
		API.updateOrCreateUser(userToSave, token['pg-token'])
			.then((savedUser) => {
				if (savedUser.group_id !== userToEdit.group_id) {
					const updatedUsersInGroup = props.usersInGroup.filter(
						(u) => u.id !== savedUser.id
					);
			...
			<div className='edit-student-container'>
				<Container className='p-0 d-flex flex-column'>
					<label className='student-name'>
						{userToEdit && userToEdit.email.split('@')[0]}
					</label>
					<Row className='mt-66'>
						<Col>
					...
					<Row className='mt-34'>
						...
						<Col>
							<DropdownButton
								className='custom-dd'
								title={userGroup}
								onSelect={(eventKey) => selectGroup(eventKey)}
							>
								{props.groups &&
									props.groups.length > 0 &&
									props.groups.map((grp) => {
										return (
											<Dropdown.Item key={grp && grp.id} eventKey={grp.id}>
												{grp.name}
											</Dropdown.Item>
										);
									})}
				...
				<div className='flex-center'>
					<footer className='student-action-btns'>
						<Button
							className='custom-sized-user-btn'
							onClick={() => setEditUserClicked(false)}
						>
							Go Back
						</Button>
						<Button
