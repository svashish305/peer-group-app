...
function GiveFeedback(props) {
	...
	const giveFeedback = () => {
		if (feedbackInput?.length > 0) {
			API.giveFeedback(
				{ remarks: feedbackInput, receiver_id: props.receiver.id },
				token['pg-token']
			)
			...
			toast.info('Feedback must not be empty!');
		}
	};
		...
		<div className='give-feedback-container'>
			<Container>
				...
				<Row>
					<Col className='p-0'>
						<textarea
							className='float-left feedback-input'
							autoFocus
							onChange={(evt) => setFeedbackInput(evt.target.value)}
						></textarea>
					</Col>
				</Row>
			</Container>
			<div className='flex-center'>
				<footer className='give-feedback-action-btns'>
					...
					<Button
						className='custom-feedback-btn'
						onClick={() => setGiveFeedbackClicked(false)}
					>
						Cancel
					</Button>
				</footer>
