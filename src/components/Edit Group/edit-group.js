...
function EditGroup(props) {
	...
	const [isCreateUserButtonClicked, setIsCreateUserButtonClicked] = useState(
		false
	);
	...
	useEffect(
		() => {
			async function fetchData() {
				const groupToEdit = await API.getGroupById(
					props.groupId,
					token['pg-token']
				).catch((err) => console.log(err));
				setGroupToEdit(groupToEdit);
				const users = await API.getUsersInGroup(
					props.groupId,
					token['pg-token']
				).catch((err) => console.log(err));
			...
			{props.editClicked ? (
				editUserClicked ? (
					<EditStudent
						...
						groups={props.groups}
						currGroup={groupToEdit}
						userId={userIdToEdit}
						onEditUserClickedChange={setEditUserClicked}
						onUsersInGroupChange={setUsersInGroup}
					/>
				) : showMeetingsClicked ? (
					<div>
						<MeetingList
							loggedInUser={props.loggedInUser}
							showMeetingsClicked={showMeetingsClicked}
							onShowMeetingsClickedChange={setShowMeetingsClicked}
							group={groupToEdit}
						/>
					</div>
				) : (
					<div className='edit-group-container'>
						{groupToEdit ? (
							<Container>
								<input
									className='group-name-input float-left'
									defaultValue={groupToEdit.name}
									onClick={(evt) => evt.target.select()}
									onKeyDown={(evt) =>
										evt.key === 'Enter' &&
										evt.preventDefault() &&
										updateGroupName(evt.target.value)
									}
												...
												<Row className='user-list-item mb-20'>
													<Col className='p-0 name-col'>
														<div className='user-name'>
															{user.name && user.name !== 'New Student'
																? user.name
													...
													<div className='p-0'>
														<Image
															className='edit-user-icon pointer'
														...
														/>
														<Image
															className='delete-user-icon pointer'
									...
									<Row>
										...
										<div className='p-0'>
											...
											<FontAwesomeIcon
												icon={faTimes}
												className='pointer'
												onClick={() => setIsCreateUserButtonClicked(false)}
											/>
										</div>
									</Row>
								) : null}
						...
						<div className='flex-center'>
							<footer className='action-btns'>
								...
								<Button
									className='custom-sized-btn'
									onClick={() => setShowMeetingsClicked(true)}
								>
									View Meetings
								</Button>
							</footer>
