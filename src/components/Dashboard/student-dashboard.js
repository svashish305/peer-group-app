	...
	useEffect(
		() => {
			async function fetchData() {
				const groupOfUser = await API.getGroupOfUser(
					props.loggedInUser.id,
					token['pg-token']
				).catch((err) => console.log(err));
				setGroupOfUser(groupOfUser);

				const usersInGroup = await API.getUsersInGroup(
					groupOfUser.id,
					token['pg-token']
				).catch((err) => console.log(err));
				...
				const oldStart = [
					props.loggedInUser.availability.split('-')[0].slice(0, 2),
					':',
					props.loggedInUser.availability.split('-')[0].slice(2),
				].join('');
				const oldEnd = [
					props.loggedInUser.availability.split('-')[1].slice(0, 2),
					':',
					props.loggedInUser.availability.split('-')[1].slice(2),
				].join('');
		...
		<React.Fragment>
			{showFeedbackClicked ? (
				<FeedbackList
					...
					showFeedbackClicked={showFeedbackClicked}
					onShowFeedbackClickedChange={setShowFeedbackClicked}
				/>
			) : showMeetingClicked ? (
				<MeetingList
					userId={props.loggedInUser.id}
					showMeetingClicked={showMeetingClicked}
					onShowMeetingsClickedChange={setShowMeetingClicked}
				/>
			) : giveFeedbackClicked ? (
				<GiveFeedback
					userId={props.loggedInUser.id}
					...
					onGiveFeedbackClickedChange={setGiveFeedbackClicked}
				/>
			) : (
				<div className='user-dashboard-container'>
						...
						<Row>
							...
							<Col>
								<div className='flex-center read-only'>
									{groupOfUser && groupOfUser.name}
								</div>
							</Col>
						</Row>
					</Container>
					<label className='mt-8 float-left label-text'>My Peers : </label>
					<br />
					{usersInGroup &&
						usersInGroup.length > 0 &&
						usersInGroup
							.filter((user) => user.id !== props.loggedInUser.id)
							.map((peer) => {
								return (
									<Row key={peer && peer.id} className='mt-20'>
										...
										<Col>
											<Image
												...
												alt='feedback'
												className='feedback-icon pointer'
												onClick={() => giveFeedback(peer)}
											/>
										</Col>
									</Row>
								);
					...
					<div>
						<label className='mt-34 float-left label-text'>
							My Preferred Meeting Slot :{' '}
						</label>
						<TimeRangePicker
							className={isDesktopOrLaptop ? 'mt-34' : 'mt-20'}
							minTime='19:00:00'
							maxTime='22:00:00'
							format='H mm'
							rangeDivider=' to '
							...
							clearIcon={null}
							value={timeRange}
							onChange={(value) => changeAvailability(value)}
						/>{' '}
						<br />
					...
					<div className='flex-center'>
						<footer className='user-action-btns'>
							...
							<Button
								className='custom-sized-btn'
								onClick={() => setShowFeedbackClicked(true)}
							>
								My Feedbacks
							</Button>
							<Button
								className='custom-sized-btn'
								onClick={() => setShowMeetingClicked(true)}
							>
								My Meetings
							</Button>
						</footer>
